<?php defined('IN_PHPCMS') or exit('No permission resources.'); ?><form method="get" name="" id="" action="<?php echo APP_PATH;?>index.php" target="_blank" onsubmit="check_search()">
			...
			<div class="select">
              <select style="display:none;" name="catid" id="headSearch" ><option selected="selected" value="0">请选择产品分类</option>
                <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=7c8e2d9694dbcc99e888814433a2727d&sql=SELECT+catname%2Ccatid+FROM+ji_category+WHERE+parentid%3D11&return=cat_pro_arr\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">修改</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT catname,catid FROM ji_category WHERE parentid=11 LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$cat_pro_arr = $a;unset($a);?>
